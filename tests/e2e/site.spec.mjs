...
for (const width of [320, 390, 768]) {
  test(`mobile navigation stays visible and usable at ${width}px`, async ({ page }) => {
    ...
    await page.screenshot({ path: `.work/evidence/home-mobile-${width}.png` })
    ...
    await page.screenshot({ path: `.work/evidence/mobile-${width}.png`, fullPage: true })
  })
}
...
test('full content without JavaScript and genuine missing-page 404', async ({ browser }) => {
  ...
  await page.screenshot({ path: '.work/evidence/ssg-without-javascript.png', fullPage: true })
...
test('desktop typography and visible content', async ({ page }) => {
  ...
  await page.screenshot({ path: '.work/evidence/home-desktop.png' })
})
