...
## Изменение от 07.09.2026 — подготовка к публикации

Новая реализация находится в `feature/production-hardening`. Проверки локальной сборки не означают, что production уже обновлён. Переход REG.RU на атомарные релизы требует однократной подготовки docroot; новый workflow до неё завершится безопасной ошибкой. Порядок: [деплой, backup, bootstrap и rollback](docs/deployment.md). [Поисковики, sitemap и аналитика](docs/search-and-analytics.md). [Серверная защита](docs/security.md). Фактическая отправка sitemap пока не выполнена: кабинеты поисковиков ещё не настроены.
