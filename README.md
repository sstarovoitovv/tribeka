...
Frontend не требует постоянно работающего Node.js-сервера. Vite формирует каталог `dist`, содержимое которого раздаётся как статические файлы. После сборки SEO-генератор создаёт отдельный HTML для каждого известного маршрута. Неизвестные URL передаются в общий `index.html`, после чего их обрабатывает React Router.
...
- `vercel.json` поддерживает канонические URL с завершающим `/` и SPA fallback.
...
3. Запускает ESLint, автоматические тесты и проверку синтаксиса PHP.
4. Создаёт production-сборку и статические SEO-страницы.
5. Подключается к REG.RU отдельным SSH-ключом.
6. Обновляет закрытые CLI-скрипты обслуживания.
7. Копирует текущую production-версию в каталог резервных копий.
8. Оставляет только 10 последних production-бэкапов.
9. Синхронизирует `dist/` с корнем сайта через `rsync --delete`.
10. Выполняет HTTP-проверку основного домена и endpoint формы.
...
Количество задаётся `BACKUPS_TO_KEEP` в workflow. `server/prune-backups.php` принимает только значения от 2 до 50 и удаляет исключительно каталоги ТРИБЕКА с проверенным форматом имени. Для безопасной проверки без удаления используйте `--dry-run`.
...
Известные маршруты получают собственный статический HTML с SEO-метаданными. Неизвестные URL требуют fallback на корневой `index.html`:
...
`.htaccess` также заставляет браузер перепроверять HTML после релиза, чтобы ранее открытая вкладка не продолжала подключать устаревший JS-файл. Удаление этих файлов приведёт к серверным 404 при обновлении внутренних страниц и проблемам с обновлением клиентского кэша.
