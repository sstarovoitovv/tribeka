...
7. Обновляет закрытые CLI-скрипты обслуживания.
8. Синхронизирует `dist/` с корнем сайта через `rsync --delete`.
9. Выполняет HTTP-проверку основного домена и endpoint формы.
