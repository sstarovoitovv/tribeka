      ...
      <div className="mx-auto grid h-[68px] max-w-xl grid-cols-4">
        {dashboardItems.map(({ label, to, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) => `group relative flex min-w-0 flex-col items-center justify-center gap-1 text-[9px] font-black uppercase tracking-[0.08em] transition-colors duration-500 ease-in-out ${isActive ? 'text-signal' : 'text-ink/50 hover:text-signal'}`}
          >
            {({ isActive }) => (
              <>
                <span className={`grid size-7 place-items-center transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${isActive ? '-translate-y-0.5' : 'translate-y-0 group-hover:-translate-y-0.5'}`}>
                  <Icon size={21} strokeWidth={isActive ? 2.4 : 2} aria-hidden="true" />
                </span>
