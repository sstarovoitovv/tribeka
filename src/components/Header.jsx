...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-[#f7f8fa]">
      <div className="mx-auto flex h-[72px] w-full max-w-[1600px] items-stretch min-[1000px]:grid min-[1000px]:grid-cols-[190px_minmax(340px,1fr)_180px_216px] min-[1000px]:pr-6 xl:grid-cols-[248px_minmax(400px,1fr)_204px_216px] xl:pr-[42px]">
        ...
        <button onClick={() => setOpen((current) => !current)} className="ml-auto mr-4 grid size-11 place-items-center self-center rounded-[9px] bg-[#e3e7ed] text-ink transition-[border-radius,background-color] duration-500 ease-in-out hover:rounded-full hover:bg-[#d7dde6] min-[1000px]:hidden" aria-label={open ? 'Закрыть меню' : 'Открыть меню'} aria-expanded={open} aria-controls="mobile-navigation">
